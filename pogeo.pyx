...
'''
cdef S2Point p1, p2, p3, p4, p5, p6
p4 = S2LatLng.FromDegrees(40.1, -110.1).ToPoint()
p3 = S2LatLng.FromDegrees(40.1, -110.6).ToPoint()
p2 = S2LatLng.FromDegrees(40.6, -110.6).ToPoint()
p1 = S2LatLng.FromDegrees(40.6, -110.1).ToPoint()
cdef vector[S2Point] *points = new vector[S2Point]()
points.push_back(p1)
points.push_back(p2)
points.push_back(p3)
points.push_back(p4)
cdef S2Loop loop
loop.Init(deref(points))


from shapely.geometry import box, Polygon
from shapely.prepared import prep
poly = box(40.1, -110.6, 40.6, -110.1)
prepped = prep(poly)

cdef S2Point spoint = S2LatLng.FromDegrees(40.3, -110.3).ToPoint()
cdef Location lpoint = Location(40.3, -110.3)

def loop_contains():
    return loop.Contains(spoint)

def poly_contains():
    return prepped.contains(lpoint)
'''
